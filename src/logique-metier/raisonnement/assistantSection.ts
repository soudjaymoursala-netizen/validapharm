...
export type { DocumentNormatifPourPrompt as DocumentNormatifPourAssistant } from '../bibliotheque-normes/formaterDocumentNormatifPourPrompt'
 ...
 *
 * `documentsNormatifs` (bibliothèque de normes) suit le même idiome,
 * factorisé avec `genererBrouillonSection` dans
 * `formaterDocumentNormatifPourPrompt` : injectés tels quels, jamais
 * résumés ou reformulés par une étape intermédiaire qui risquerait de
 * fabriquer du contenu. Aucun filtrage de pertinence ici : c'est au
 * modèle, pas à un heuristique local non prouvé, de décider ce qui
...
export function construireObjectifAssistantSection(
  ...
  return [
    ...
    contenuSection,
    blocDocumentsNormatifsPourPrompt(documentsNormatifs),
    '',
