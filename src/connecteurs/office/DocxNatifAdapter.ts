...
/**
 * Ingestion native d'un document `.docx` — première brique du chantier
 * "ingestion Office native" (texte, puis images incorporées).
 *
 ...
 * initialement envisagée : `mammoth` distingue son code Node
 ...
 * (`connecteurs/ocr/OcrRelayAdapter.ts`) — jamais un
 ...
 * `SourceVersion` (`Extraction produces ExtractionItem 1:N`,
 ...
 * PWA-only.
 *
 * **Limite assumée et non résolue** : un scan filigrané (SOP
 ...
 * un texte dégradé par un filigrane doit être visible et
 * corrigé à cette étape, jamais silencieusement accepté.
 *
 * @requirement Ingestion Office native (.docx)
 */
...
/**
 * Extrait les tableaux d'un `.docx` en grille de cellules brutes —
 * calibré sur un manuel équipement réel lu dans Google
 ...
 * par le parseur de structure de SOP à en-têtes numérotés.
 *
...
/** Types d'image raster que le relais OCR sait traiter. */
