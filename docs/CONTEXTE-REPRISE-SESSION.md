...
- **Méthodologie d'audit/montée en maturité (ajoutée le 24/08/2026, ne pas re-décider)** : l'utilisateur a fourni un "prompt maître d'audit GxP/IA" exigeant (15 équipes d'audit virtuelles, mentalité zéro complaisance/"montrez-moi la preuve"/no fake evidence). Conservé verbatim dans `docs/audit/PROMPT-MAITRE-AUDIT-MATURITE.md`, décision d'application détaillée en `00-cadrage-projet.md` §6quater. **Résumé de la décision** : pas d'audit formel complet maintenant (reporté à quand il y aura une IA/sécurité/infra réelles à auditer) ; sa mentalité (zéro complaisance, corriger réellement les gaps trouvés, pas de conformité cosmétique) s'applique en revanche **dès maintenant, en continu**, dans le cycle normal de construction. Ne pas reproposer cette décision, l'appliquer.
...
Code : conception démarrée le 23/08/2026, premier incrément vertical réellement écrit et testé le 24/08/2026 (même session que ce fichier), poursuivi toute la journée du 24/08/2026. Toolchain validé (lint/typecheck/format/test/build OK) : TypeScript strict + Vue 3 + Pinia + Vue Router + Dexie.js + Vitest + ESLint/Prettier (`vite-plugin-pwa` pas encore câblé — backlog). Structure en couches stricte (`presentation/` / `logique-metier/` / `connecteurs/` / `persistance/`), imposée par une règle ESLint. Livré et poussé (PR #1, CI verte) : modèle de données pivot, garde de compatibilité de schéma, machine à états du cycle de vie d'une section, garde-fous de finalisation (U-01/U-02/U-03), calcul IPR, persistance Dexie, 6 écrans (Tableau de bord/Fiche Projet/Éditeur de section/Blocage U-12/Configuration client GitHub/Résolution de conflit/Gestion des clients/Configuration Drive — 8 en tout), connecteur GitHub réel (lecture/écriture groupée/résolution de conflit par SHA, SDS §5) et connecteur Drive réel (miroir, SDS §5bis) testés contre `fetch` mocké, orchestrateur de synchronisation GitHub (push/pull) et orchestrateur de miroir Drive par client (déclenchement manuel "Sauvegarder maintenant"), entité `client` (FS §3 v12, gap trouvé en construisant la config Drive par client_id). 148 tests. Migration de schéma Dexie v1→v2 vérifiée en navigateur réel sans perte de données (condition non négociable). Moteur de gabarits déclaratif (FDS §4) livré ensuite : validation par type de champ (FDS §6), colonne calculée câblée à `calculerIPR` (FDS §5), composant `RenduGabarit.vue` remplaçant le champ "contenu" générique de l'éditeur de section quand un gabarit existe (repli inchangé sinon) ; 3 gabarits réels (contexte_procede, dq avec table de risques IPR, plan_metrologie). 168 tests. Export/import de section (FS §4.3) livré ensuite : JSON (URS-F-021, import "transfert entre postes" créant toujours une section nouvelle), Word .doc HTML (URS-F-020), impression/PDF sans coupure de tableau en milieu de ligne, CSV par tableau dynamique (URS-F-022), blocage/forçage d'export (URS-F-027). Gap trouvé et corrigé au passage : le rappel obligatoire "validé en interne — pas une signature électronique opposable" (URS-F-011bis) n'était affiché nulle part (ni écran ni export) — `logique-metier/i18n/libellesStatut.ts` créé et câblé aux deux. 201 tests. Backlog restant : gabarits manquants du catalogue (8 TemplateType déjà utilisables sans définition + reste du catalogue URS §10) et génération de brouillon par adaptation IA (§4.1bis), gabarits d'export personnalisés par client (§4.3bis), chat IA/relais, Structure Système/QMS, assistant de qualification/bibliothèque de normes, déclenchement automatique du miroir Drive par heuristique de fin de session (le déclenchement manuel est fait).
