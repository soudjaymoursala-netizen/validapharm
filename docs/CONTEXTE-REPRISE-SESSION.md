...
| URS | **v30** (`01-URS-outil.md`) — §4.6/URS-F-050 corrigé Phase 1 ; §4.6bis/§4.6ter (F1/F3) Phase 3 ; §4.10bis/URS-F-103 (Function/Process/ManufacturingContext) Phase 4 ; §4.6quater/URS-F-058 (Parameter/CPP/CQA, backfill Phase 2 oublié à l'époque) et §4.11/URS-F-110 (Quality Events) Phase 5 ; collision d'ID URS-F-040 corrigée (renumérotée URS-F-039bis) |
| Analyse de risque (AR) | **v28** (`02-analyse-de-risque-outil.md`) — R-15/R-16 terminologie corrigée Phase 1 |
| FS | **v20** (`03-specifications-fonctionnelles.md`) — §4.6 corrigé Phase 1, §4.6bis/§4.6ter Phase 3, §4.10bis Phase 4, §4.6quater/§4.11 Phase 5 |
...
- **362 tests, 50 fichiers de test, tous verts** (`npx vitest run`, vérifié le 25/08/2026) — inclut désormais `workers/ocr-relay/**` (Cloudflare Worker, code serveur), intégré à la même suite Vitest/tsc/ESLint racine (voir `vite.config.ts` `test.include`, `tsconfig.json` référence `workers/ocr-relay`) sans fragmenter le pipeline.
- Toolchain : TypeScript strict + Vue 3 + Pinia + Vue Router + Dexie.js (IndexedDB, schéma **v11**) + Vitest + ESLint/Prettier. Structure en couches stricte (`presentation/` / `logique-metier/` / `connecteurs/` / `persistance/`), imposée par une règle ESLint. `workers/` (nouveau, Phase 6) est délibérément hors de `src/` : code serveur déployé indépendamment sur Cloudflare, jamais bundlé dans la PWA.
...
**Phase 7 — Test/Execution/Evidence engine** : remplace les tableaux libres FAT/SAT/IQ/OQ/PQ par de vraies entités traçables. Dépend de la Phase 3 (Assessment/Requirement/Risk) déjà terminée. Risque élevé signalé dans `CONVERGENCE_PLAN.md` ("chantier long... à séquencer en sous-étapes, jamais en un seul commit") — à traiter avec prudence, probablement sur plusieurs incréments.
