...
- **370 tests, 51 fichiers de test, tous verts** (`npx vitest run`, vérifié le 25/08/2026) — inclut désormais `workers/ocr-relay/**` (Cloudflare Worker, code serveur), intégré à la même suite Vitest/tsc/ESLint racine (voir `vite.config.ts` `test.include`, `tsconfig.json` référence `workers/ocr-relay`) sans fragmenter le pipeline.
- Toolchain : TypeScript strict + Vue 3 + Pinia + Vue Router + Dexie.js (IndexedDB, schéma **v12**) + Vitest + ESLint/Prettier. Structure en couches stricte (`presentation/` / `logique-metier/` / `connecteurs/` / `persistance/`), imposée par une règle ESLint. `workers/` (nouveau, Phase 6) est délibérément hors de `src/` : code serveur déployé indépendamment sur Cloudflare, jamais bundlé dans la PWA.
