...
- **339 tests, 47 fichiers de test, tous verts** (`npx vitest run`, vérifié le 25/08/2026).
- Toolchain : TypeScript strict + Vue 3 + Pinia + Vue Router + Dexie.js (IndexedDB, schéma **v10**) + Vitest + ESLint/Prettier. Structure en couches stricte (`presentation/` / `logique-metier/` / `connecteurs/` / `persistance/`), imposée par une règle ESLint.
...
**Phase 6 — Extension serverless (Cloudflare Workers) pour OCR/recherche** : déjà résolue architecturalement (TD-001, 25/08/2026) — reste à construire réellement le second Worker sans état. Détail complet dans `CONVERGENCE_PLAN.md`.
