...
### 4.3bis Gabarits d'export personnalisés (templates client) *(implémenté v54 — Phase 26 de convergence architecturale, 27/08/2026, `docs/convergence/PHASE_26_GABARITS_EXPORT_CLIENT_SPEC.md`, TD-024)*

| ID | Exigence | Priorité |
|---|---|---|
| URS-F-023 | Le système DEVRAIT permettre d'associer à un client/organisation un gabarit d'export personnalisé (Word/PDF/Excel) fourni par l'utilisateur, en plus du gabarit par défaut de l'outil. **Portée réellement livrée (Phase 26)** : gabarit `.docx` uniquement (`docxtemplater`+`pizzip`) — PDF/Excel restent hors périmètre de ce lot, aucune librairie de génération saine identifiée pour ces deux formats à ce jour (jamais silencieusement présumés couverts). | Should |
...
| URS-F-025 | Lorsqu'un gabarit personnalisé est utilisé, le système DOIT produire un document dont le contenu est identique à celui produit avec le gabarit par défaut — seule la mise en forme diffère. Un test de non-régression DOIT vérifier cette équivalence de contenu. **Réalisé** par une fonction de construction de données unique (`construireDonneesExportGabarit`) consommée par les deux renderers (HTML par défaut et `.docx` personnalisé) — équivalence garantie par construction, jamais deux lectures indépendantes des mêmes données. | Must |
| URS-F-026 | Lors de la configuration d'un gabarit personnalisé, le système DOIT permettre de vérifier que les éléments obligatoires (bloc de signatures, historique des révisions) sont bien mappés dans le template client. **Réalisé** : un gabarit qui ne mappe pas les balises `redacteurs`/`approbateur_final`/`historique_revisions` est **refusé à l'import**, jamais enregistré "à corriger plus tard". | Must |
...
| URS-F-160octies | *(nouveau v57 — Phase 28 de convergence architecturale, TD-026)* `readiness` DOIT être calculé automatiquement à la création d'un `ContentPlan`, à partir de la chaîne réelle `Requirement → Couverture → Test → Execution → Evidence` (§4.12-§4.14) ancrée sur `asset_node_id`, complétée d'une vérification de `QualityEvent` bloquant sur ce même nœud — jamais fourni librement par l'appelant. | Must |
| URS-F-160nonies | Le système DOIT permettre de recalculer `readiness` à la demande sur un `ContentPlan` non gelé, pour refléter des `Execution`/`Evidence` apparues après sa création — action explicite, tracée dans son journal d'audit. | Should |
...
### 4.20bis Context Engine enrichi — narratif + grounding réel *(nouveau v56 — Phase 27 de convergence architecturale, 27/08/2026 ; spec `docs/convergence/PHASE_27_CONTEXT_ENGINE_ENRICHI_SPEC.md`, TD-025)*

| ID | Exigence | Priorité |
|---|---|---|
| URS-F-201 | Le système DOIT organiser les éléments déjà résolus d'un `ContextSnapshot` (§4.20) en un narratif à quatre facettes — OÙ (localisation), QUOI (faits connus), COMMENT (procédure applicable), POURQUOI/IMPACT (événements qualité en cours) — plutôt que la seule liste plate d'objets typés existant jusqu'ici. | Should |
| URS-F-201bis | Lorsqu'un `ContextSnapshot` est en vigueur pour une invocation du moteur de raisonnement (§4.21), son narratif DOIT être injecté dans le prompt envoyé au fournisseur IA — le contexte assemblé DOIT réellement influencer ce que le modèle voit, jamais rester une simple trace d'audit (`AIRequest.context_snapshot_id`) sans effet sur le raisonnement lui-même. | Must |
| URS-F-201ter | Les identifiants d'objets portés par le narratif de contexte DOIVENT être traités avec la même garantie qu'un identifiant obtenu par un appel d'outil pour la vérification de citation déterministe (§4.21ter/URS-F-210ter) — une citation vers un fait du narratif ne DOIT jamais être rétrogradée au seul motif qu'aucun appel d'outil ne l'a explicitement redemandé. | Must |
| URS-F-201quater | **Limite assumée** : la facette COMMENT (procédure applicable) reste vide dans ce lot — `Procedure` (§4.25) n'a aucun rattachement `Workspace`/`AssetNode` à ce jour (limite déjà documentée, §4.25/URS-F-250quater) ; l'ajouter sans cas réel démontré fabriquerait une résolution non éprouvée. Les facettes QUOI (`ManufacturingContext`) et POURQUOI/IMPACT (`QualityEvent`) sont vérifiées par test unitaire uniquement — aucun écran de création n'existe à ce jour pour ces deux entités (limite préexistante, non introduite par ce lot). | — |
...
### 4.27 Lecture de tableaux Word + fournisseur Document Intelligence disponible *(nouveau v51 — Phase 22 de convergence architecturale, 27/08/2026 ; spec `docs/convergence/PHASE_22_TABLEAUX_ET_DOCUMENT_INTELLIGENCE_SPEC.md`, TD-019/TD-020 ; en réponse à la demande explicite de l'utilisateur de vérifier dans tous les catalogues npm l'existence d'outils de lecture de tableaux)*

| ID | Exigence | Priorité |
|---|---|---|
| URS-F-270 | Le système DOIT pouvoir extraire les tableaux d'un `.docx` (grille de cellules brutes, plus le texte du paragraphe non vide le plus proche précédant chaque tableau) — sans appel à une librairie tierce non saine ni à un modèle IA. | Should |
| URS-F-270bis | Le système DOIT proposer une étape candidate pour toute ligne d'un tableau dont la première cellule est exactement un numéro d'étape — combinant le titre précédent et les préconditions détectées dans le même tableau (ex. "Previous achievement"/"Required time") en contexte, sans jamais fabriquer une précondition absente. Ces étapes s'ajoutent aux étapes textuelles déjà proposées (§4.26), chaque tableau conservant sa propre numérotation d'origine. | Should |
| URS-F-270ter | Le système DOIT rendre disponible un fournisseur OCR capable de reconstruire la structure d'un tableau depuis un scan/PDF (Azure AI Document Intelligence), conforme au même contrat que le fournisseur actif — sans l'activer automatiquement : le remplacement ou l'ajout d'un fournisseur OCR reste une décision explicite de l'utilisateur (compte Azure, quota, coût potentiel). | Should |
| URS-F-270quater | **Limites découvertes, assumées** : aucune ingestion PDF réelle n'existe dans la PWA (testé uniquement via le rendu Google Drive, non représentatif d'une vraie extraction PDF) ; une ligne de données numériques dans un tableau (ex. valeurs de calibration) peut être faussement reconnue comme un en-tête de section (classée "autre", jamais un mauvais canon) ; un tableau imbriqué dans une cellule n'est pas extrait ; le contrat REST du fournisseur Document Intelligence n'a pas pu être revérifié en environnement réel (documentation Microsoft inaccessible durant cette session). | — |

### 4.28 Ingestion PDF native *(nouveau v52 — Phase 23 de convergence architecturale, 27/08/2026 ; spec `docs/convergence/PHASE_23_INGESTION_PDF_SPEC.md`, TD-021 ; en réponse à la demande explicite de l'utilisateur "Fais les deux [ingestion PDF + repli IA]. Tu trouves pas des sop en pdf dans Google drive ?")*

| ID | Exigence | Priorité |
|---|---|---|
| URS-F-280 | Le système DOIT pouvoir extraire le texte brut d'un `.pdf` natif (page par page, pages jointes dans l'ordre) — sans appel réseau, sans appel à un modèle IA. | Should |
| URS-F-280bis | Le système DOIT retenir une seule et même implémentation de parsing entre l'environnement de test et l'environnement de production — seule la résolution d'URL d'assets auxiliaires (script worker, polices standard) peut différer entre les deux, jamais la logique d'extraction elle-même. | Must |
| URS-F-280ter | Un fichier qui n'est pas un PDF valide ou illisible DOIT lever une erreur typée explicite, jamais un crash silencieux ni un texte vide non signalé. | Must |
| URS-F-280quater | **Limites assumées** : aucune extraction de structure de tableau depuis un PDF natif (seul le texte plat est extrait — un PDF scanné avec tableaux reste couvert par le repli Document Intelligence, §4.27) ; aucun wiring écran/store dans ce lot. Un quatrième genre réel de SOP (manuel équipement, en-têtes en gras sans numérotation) découvert pendant cette phase confirme que la couverture déterministe de "tous les types" de SOP reste hors de portée d'un système de règles (§4.26/URS-F-260septies). | — |

### 4.29 Repli IA-assisté de structuration procédurale *(nouveau v53 — Phase 24 de convergence architecturale, 27/08/2026 ; spec `docs/convergence/PHASE_24_REPLI_IA_STRUCTURE_PROCEDURE_SPEC.md`, TD-022 ; point ouvert depuis §4.25/URS-F-250quater (TD-016), explicitement engagé par l'utilisateur)*

| ID | Exigence | Priorité |
|---|---|---|
| URS-F-290 | Pour un document hors couverture du parseur déterministe (§4.26-§4.27), le système DOIT pouvoir proposer une structure (sections + étapes) via un fournisseur IA existant, en exigeant explicitement du modèle qu'il recopie le texte source mot pour mot plutôt que de le reformuler ou le résumer. | Should |
| URS-F-290bis | Chaque section/étape proposée par l'IA DOIT être vérifiée déterministiquement contre le document source (texte normalisé) avant de recevoir un état de confiance `'infere'` — un texte non retrouvé dans la source (reformulation ou hallucination) DOIT rester `'a_verifier'`, jamais un état plus confiant sur la seule affirmation du modèle. | Must |
| URS-F-290ter | La réponse brute du modèle DOIT être conservée même si son analyse échoue partiellement ou totalement (traçabilité minimale) ; une ligne de réponse mal formée DOIT être ignorée sans interrompre le traitement des lignes suivantes. | Must |
| URS-F-290quater | **Garde-fou non négociable, inchangé (TD-016)** : le résultat de ce repli n'est jamais écrit dans `Procedure`/`ProcedureStep` sans confirmation humaine explicite. **Limites assumées** : la vérification d'ancrage est une comparaison de sous-chaîne normalisée, pas une similarité sémantique floue — une reformulation légère reste honnêtement `'a_verifier'`. Aucun déclenchement automatique ni écran de revue construits dans ce lot. | — |

### 4.30 Écran de revue de structure procédurale + déclenchement du repli *(nouveau v54 — Phase 25 de convergence architecturale, 27/08/2026 ; spec `docs/convergence/PHASE_25_ECRAN_REVUE_STRUCTURE_PROCEDURE_SPEC.md`, TD-023 ; clôture le point ouvert §4.29/URS-F-290quater)*

| ID | Exigence | Priorité |
|---|---|---|
| URS-F-300 | Le système DOIT orchestrer automatiquement le parseur déterministe (§4.26-§4.27) puis, uniquement si celui-ci ne retourne strictement aucune section ni aucune étape, le repli IA-assisté (§4.29) — jamais les deux invoqués systématiquement, et jamais un résultat mélangeant les deux provenances dans une même proposition. | Must |
| URS-F-300bis | Le système DOIT fournir un écran permettant de fournir le texte d'une SOP (collé ou importé depuis un fichier `.docx`/`.pdf` réutilisant l'ingestion native existante, §4.15/§4.28), de déclencher la génération d'une proposition de structure, de revoir et modifier chaque section/étape proposée (y compris exclure une étape), et de confirmer — la confirmation seule DOIT créer réellement la `Procedure` et ses `ProcedureStep` retenues. | Must |
| URS-F-300ter | Chaque élément d'une proposition issue du repli IA DOIT afficher son état de confiance (`EtatConfianceIA`) de façon visuellement distincte, jamais confondu avec le statut de qualification d'un autre domaine (même garde-fou que §4.23, TD-010). | Must |
| URS-F-300quater | **Garde-fou non négociable, inchangé (TD-016)** : aucune fonction de génération de proposition n'écrit directement dans `Procedure`/`ProcedureStep` — seule une confirmation humaine explicite via l'écran le permet. **Limite assumée** : aucune réconciliation entre une nouvelle proposition et une `Procedure` existante de même référence (confirmer crée toujours une nouvelle version) ; aucune sauvegarde de brouillon d'édition entre deux sessions. | — |
