...
| URS-F-120ter | Le système DOIT permettre de proposer un ou plusieurs candidats de test (`TestCandidate`) pour un objectif de test, avec un statut explicite (proposé / retenu / écarté). | Should |
| URS-F-120quater | *(garde-fou non négociable, Must)* Un test formel (`Test`) NE DOIT JAMAIS être créé à partir d'un candidat qui n'est pas au statut "retenu" — jamais depuis un candidat proposé ou déjà écarté. | Must |
...
| URS-F-130sexies | Le système DOIT permettre de consigner un événement survenu pendant l'exécution (anomalie, pause, reprise, commentaire), avec une référence optionnelle vers un événement qualité (`QualityEvent`, §4.11) déjà existant — cette référence NE DOIT JAMAIS être créée automatiquement (cohérent avec DEC-002/DEC-055, §4.11). | Should |
...
### 4.15 Source/Document Intelligence — structuration assistée *(nouveau v34 — Phase 8a de convergence architecturale, 25/08/2026 ; spec `docs/convergence/PHASE_8A_SOURCE_INTELLIGENCE_SPEC.md` ; séquencement TD-004, sous-phase 8b — compréhension de schémas techniques complexes — non engagée)*

| ID | Exigence | Priorité |
|---|---|---|
| URS-F-150 | Le système DOIT permettre de référencer une source documentaire (`Source`, document ou image) et d'y rattacher un texte extrait (`Extraction`, via OCR ou saisie manuelle) — l'`Extraction` reste immutable une fois enregistrée. | Should |
| URS-F-150bis | *(garde-fou non négociable, Must)* Toute interprétation structurée d'une `Extraction` (`KnowledgeItem`) DOIT être créée au statut "à valider" (NEEDS_REVIEW) — jamais "validé" à la création, quel que soit le contenu. | Must |
| URS-F-150ter | *(garde-fou non négociable, Must)* Le passage d'un `KnowledgeItem` au statut "validé" ou "rejeté" DOIT toujours résulter d'une action humaine explicite et tracée — jamais automatique. | Must |
...
### 4.16 ContentPlan — planification d'un livrable *(nouveau v35 — Phase 9 de convergence architecturale, 25/08/2026 ; spec `docs/convergence/PHASE_9_CONTENT_PLAN_SPEC.md` ; ne couvre que la planification, pas la génération/le rendu/l'approbation finale du livrable)*
