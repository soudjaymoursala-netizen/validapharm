...
| **Version** | 22 (dette explicitement acceptée résorbée : performance/capacité, lecteur d'écran, désinstallation/rollback — checklist §6ter) |
| **Statut** | En rédaction |
...
| URS-NF-030 | Chaque modification DOIT être attribuable et horodatée, via l'historique Git signé (GPG/SSH, coût nul) et protégé contre la réécriture (branche principale protégée, dès la Phase 1). Piste d'audit raisonnable en Phase 1 mono-utilisateur ; pas un audit trail Part 11 complet (Phase 3). | Must |
...
| URS-NF-055 | *(nouveau v22 — checklist de complétude §6ter)* La désinstallation de l'outil DOIT être réalisable par simple suppression du dossier applicatif local, sans reliquat (pas de service installé, pas de modification de configuration système en dehors de ce dossier et du dépôt Git dédié) — cohérent avec l'architecture locale-first. | Should |
...
| URS-NF-044 | *(nouveau v05 — revue technique E5)* Aucun secret (clé API, jeton d'authentification) NE DOIT être stocké en clair dans le dépôt Git ni dans le code source — exclusivement dans un mécanisme de configuration locale non versionné. | Must |
...
4. Données retrouvées à l'identique après clonage sur un second poste (commits signés vérifiables).
...
---
*Document vivant, version 22 — v06-v13 : voir historique complet dans le corps du document et les REV/AUDIT associés. v16 connecteurs QMS tiers. v17 Structure Système (référentiel d'actifs, arbre + graphe). v18 dossier vivant d'un actif. v19 export PDF de l'historique de qualification. v20 intègre un cinquième nouveau besoin exprimé par l'utilisateur (`REV-URS-VALIDAPHARM-2026-009`) : marquage "soumis à qualification périodique" avec date limite, et statut de qualification standardisé par nœud. v21 (23/08/2026, `REV-URS-VALIDAPHARM-2026-010`) : charte graphique et identité visuelle (URS-NF-054 à 054quinquies). **v22 (23/08/2026) résorbe les trois gaps mineurs actés comme dette explicitement acceptée au cadrage §6ter** : performance/capacité (URS-NF-052bis : cibles chiffrées, 2s/volume de référence 500 projets), lecteur d'écran (URS-NF-050bis), désinstallation/rollback applicatif (URS-NF-055/055bis — dont une protection réelle contre la corruption de données au rollback vers une version incompatible, trouvée en traitant ce point). FS/FDS/SDS à mettre à jour en conséquence.*
