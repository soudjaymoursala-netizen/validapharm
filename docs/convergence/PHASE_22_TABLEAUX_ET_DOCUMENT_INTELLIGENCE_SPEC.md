...
- **Ferring "SOP Qualif Balance.pdf"** (même contenu que SMP-PROC-016168 déjà utilisé précédemment): les 9 sections canoniques toutes détectées correctement (objectif/perimetre/definitions/responsabilites/procedure/references/documentation×2), avec **3 sections `'autre'` parasites** au milieu — des lignes de données numériques dans un tableau de calibration (ex. "5 000 10 000") faussement reconnues comme en-têtes de premier niveau. Limite réelle découverte, jamais perdue silencieusement (toujours `'autre'`, jamais un mauvais canon), mais fragmente la section "Procédure".
