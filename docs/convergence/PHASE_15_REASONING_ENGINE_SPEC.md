...
*26/08/2026 — suite des Phases 13/14, décisions d'entrée déjà tranchées par la revue panel (`PHASE_13_17_REVUE_PANEL_MOTEUR_RAISONNEMENT.md`) : TD-007 (orchestration côté navigateur, relais reste un simple proxy sans état) et TD-008 (états de confiance discrets, jamais un score numérique).*

## 1. Ce qui existe déjà (Comprendre)

- `ProviderAdapter.envoyerMessage(mode, contexte, question) → Reponse { texte, version_moteur, citations }` (SDS §6) — un appel **à un seul tour**, sans notion d'outil ni d'historique de conversation. Le corps du relais serverless (Cloudflare Worker) **n'est pas dans ce dépôt** — seule son URL est configurée (`EnregistrementConnexionRelaisIA`). Conséquence directe et vérifiée : **impossible d'étendre le protocole du relais** pour un appel d'outils natif (function calling côté fournisseur) sans toucher un système hors de ce dépôt — ce qui confirme et renforce TD-007 plutôt que de le remettre en cause.
...
- `ContextSnapshot`/`ContextSnapshotItem` (Phase 14) : assemblage de contexte réutilisable par une `Mission`.
- `Mission`/`Activity` (Phase 13).
...
Le plan initial (`CONVERGENCE_PLAN.md` §Phase 15) prévoyait une "boucle d'orchestration lisant les stores existants... comme des outils". Un appel d'outils fiable suppose normalement un support natif côté fournisseur (function calling Claude/OpenAI). Or :

- Le relais est un **simple proxy texte** (`{texte, version_moteur, citations}`), son code serveur n'est pas modifiable ici (TD-007 l'a déjà acté).
- Construire un nouveau canal d'appel d'outils natif reviendrait soit à modifier un système hors dépôt, soit à fabriquer un nouveau backend — **interdit par TD-001/TD-007**.

**Décision de conception (dans le périmètre déjà tranché par TD-007, pas une nouvelle décision de panel)** : le protocole d'appel d'outils est **entièrement textuel, défini et interprété côté navigateur** — le modèle est instruit (dans le texte de la question) de répondre soit par un appel d'outil (`APPEL_OUTIL: {...}`), soit par une réponse finale structurée (`REPONSE_FINALE: {...}`) ; l'orchestrateur reconstruit la conversation en concaténant le transcript à chaque tour (le relais reste sans état, TD-007). Si le modèle ne respecte pas le format (réponse libre), **dégradation gracieuse** : le texte brut devient la réponse finale, état de confiance forcé à `a_verifier`, jamais un crash ni une réponse fabriquée (cohérent avec la dégradation A3 déjà actée par TD-007).
...
- **Garde-fou non négociable (repris du plan)** : aucune fonction de ce lot n'écrit le contenu d'une `AIResponse` dans `Requirement`/`Test`/`KnowledgeItem` — vérifié par test de régression dédié (même principe que `Confirmation`, Phase 8a).
...
- Aucun écran : comme les Phases 5/8a/9/10/13/14 — la Phase 17 (Mission workspace) exposera le raisonnement visuellement.
