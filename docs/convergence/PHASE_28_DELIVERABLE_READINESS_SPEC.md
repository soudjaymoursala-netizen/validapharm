...
C'est un gap structurellement identique à celui découvert pour le Context Engine: un mécanisme documenté depuis longtemps comme *devant* être calculé, mais jamais construit.
...
**Conclusion honnête**: les 4 états `pret`/`besoin_information`/`besoin_revue`/`bloque` proviennent du type domaine déjà posé, pas de §26 lui-même. Cette phase construit le *calcul* de ces états déjà existants; elle n'invente ni ne corrige leur sémantique, et ne prétend pas citer §26 pour autre chose que la position de l'étape dans le pipeline.
...
- Les signaux (un par `Requirement`, et par `Execution` si plusieurs) sont combinés par **sévérité pire-cas** (`bloque` > `besoin_information` > `besoin_revue` > `pret`) — jamais une moyenne, jamais le premier trouvé. Même style que `grilleDecision.ts`/`verifierBlocageExport.ts`, déjà établis dans le code base.

Résolution du même patron déjà éprouvé par l'outil `lister_requirements_pour_actif` du Reasoning Engine et par `assemblerElementsContextSnapshot` du Context Engine: ancrage sur `asset_node_id`, aucune nouvelle résolution inventée.
...
**Impossible dans ce lot** — aucun écran `ContentPlan` n'existe dans l'application (limite déjà documentée dans `PHASE_9_CONTENT_PLAN_SPEC.md`, jamais close depuis). Vérifié par recherche exhaustive (`grep -rn "useContentPlanStore"`): seuls le store et ses tests le référencent, aucun composant `.vue`. Ce module reste donc couvert uniquement par sa suite de tests, cohérent avec la discipline déjà appliquée.
