...
- `docxtemplater`+`pizzip` : pré-choisis **par anticipation** en TD-014 (Phase 19) sans être installés — recherche re-vérifiée ici (registre npm réel, `npm audit` réel) plutôt que présumée toujours valable : 0 vulnérabilité, licences MIT confirmées, dépendances uniques (`@xmldom/xmldom` pour `docxtemplater`, `pako` pour `pizzip`).
- `genererExportWord.ts` (Phase 1/13, HTML `.doc`) : refactoré pour consommer une nouvelle fonction de construction de données partagée, jamais réécrit dans sa logique de rendu.
- `EditeurSection.vue` : écran d'export existant (§4.3), étendu plutôt que dupliqué.
- Patron d'isolation stricte par `client_id` déjà établi (`AssetNode`, `Procedure`, Phase 20/16) : reproduit à l'identique pour `GabaritExportClient`.
...
- Suite complète verte (87 fichiers / 602 tests, 19 nouveaux : 9 sur `GenerationDocxAdapter`, 4 sur `useGabaritExportStore`, 6 sur `useProcedureStore`/`proposerStructureProcedureAvecRepli` déjà comptés en Phase 25), typecheck et lint propres, `npm audit` 0 vulnérabilité sur `docxtemplater`/`pizzip`.
