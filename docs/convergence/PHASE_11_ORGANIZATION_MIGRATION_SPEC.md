...
- Réécriture des ~25 tables existantes pour interroger explicitement par `Workspace`/site (ex. "toutes les données du site X uniquement") — chantier ultérieur, phase par phase, jamais en un seul commit (cohérent avec). Elles continuent de fonctionner à l'identique via `client_id` = `Organization.id`.
