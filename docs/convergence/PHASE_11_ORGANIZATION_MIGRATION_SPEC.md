...
- **E3 (QA Réglementaire, intégrité des données)** : la migration `Client → Organization` DOIT être un acte explicite et tracé (jamais automatique/silencieux au démarrage) — cohérent avec le principe déjà appliqué au garde-fou de rollback de schéma (URS-NF-055bis). Idempotente : migrer deux fois le même client ne doit rien dupliquer.
