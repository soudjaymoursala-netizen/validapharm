...
- `Procedure`/`ProcedureStep`: structure de destination versionnée, immuable, saisie humaine.
- `extraireTexteDocx`: produit déjà le texte brut d'une SOP `.docx`; l'OCR produit le texte brut d'une SOP scannée. Les deux alimentent ce parseur de la même façon (texte brut en entrée, indépendamment de la source).
