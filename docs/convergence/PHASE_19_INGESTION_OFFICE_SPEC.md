# Phase 19 — Ingestion Office native (lecture .docx)

*27/08/2026 — deuxième phase du plan `docs/convergence/VISION_NORTH_STAR_CONVERGENCE.md`, priorité P0. Complétée le même jour (§6/§7, TD-015) suite à une observation explicite de l'utilisateur : une SOP réelle n'est pas toujours du texte seul (schémas/photos incorporés) ni toujours un `.docx` propre (scan avec ou sans filigrane) — l'ingestion doit être robuste à ces deux réalités.*

## 1. Ce qui existe déjà (Comprendre)

- OCR image (`connecteurs/ocr/OcrRelayAdapter.ts`, Phase 6) : seul mécanisme d'ingestion de document à ce jour, nécessite un relais réseau et ne lit qu'une image/scan.
- `Source → SourceVersion → Extraction → ExtractionItem → KnowledgeItem` (Phase 8a) : la chaîne de structuration existe, mais `Extraction.methode` (`MethodeExtraction`) ne connaissait que `'ocr_azure' | 'saisie_manuelle'` — aucune ingestion native d'un fichier Office.
...
## 2. Comparer — recherche de librairie (TD-014)
...
## 3bis. Robustesse de lecture (TD-015, complément du 27/08/2026)
...
- **Combinaison prévue avec l'OCR existant (Phase 6)** : chaque image extraite est destinée à être transmise à `OcrRelayAdapter.extraireTexte(donnees, contentType)`, produisant sa propre `Extraction` sur la même `SourceVersion` que le texte natif (`docx_natif`) — le modèle Phase 8a admet déjà plusieurs `Extraction` par version (`Extraction produces ExtractionItem 1:N`), aucune modification de schéma nécessaire.
- **SOP scannée avec ou sans filigrane** : un scan entier (pas un `.docx`) relève déjà de l'OCR existant (`ocr_azure`), pas de cette phase. Un filigrane ("COPIE CONTRÔLÉE"/"DRAFT") peut dégrader la reconnaissance — **aucune détection/correction automatique n'est construite ici, et il ne faut jamais en prétendre une sans preuve technique réelle** (TD-015). Le filet de sécurité reste la validation humaine déjà non négociable de tout `KnowledgeItem` (`a_valider` à la création, Phase 8a) : un texte dégradé par un filigrane doit être visible et corrigé à cette étape.

## 4. Explicitement non construit (limite assumée)

- Aucune ingestion Excel native (`xlsx_natif`) — bloquée par l'absence de librairie propre à ce jour (TD-014). Ne pas réintroduire `xlsx`/SheetJS tel quel sans correctif vérifié.
...
- Aucun wiring dans un écran ni dans `useSourceIntelligenceStore` — même discipline que l'OCR (Phase 6), qui n'est pas non plus appelé depuis une UI à ce jour ; les adaptateurs sont prêts à être invoqués par un futur écran (même rôle que `OcrRelayAdapter`).
...
- Aucune détection/correction de filigrane ou de scan dégradé (TD-015) — limite assumée et documentée, jamais un algorithme fabriqué sans preuve.
