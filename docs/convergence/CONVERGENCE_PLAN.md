...
| 23 (Template Intelligence généralisée — génération `docxtemplater`, ingestion Excel) | Non engagée — chantier P0 suivant du plan Vision North Star, voir `VISION_NORTH_STAR_CONVERGENCE.md` §4 | — | — |
| Repli IA-assisté pour la structuration procédurale (documents hors couverture du parseur déterministe/tableaux) | **Point ouvert, non engagé** — à engager seulement si un cas réel démontre que la couverture déterministe (Phases 21-22) est insuffisante, jamais par anticipation (`PHASE_22_TABLEAUX_ET_DOCUMENT_INTELLIGENCE_SPEC.md` §5, TD-017/TD-018) | — | — |
