...
**Débat**
- **E5 (Architecte)** : A2 viole directement TD-001 (extension serverless plutôt que backend complet, décision déjà actée le 25/08/2026) sans qu'aucun fait nouveau ne justifie de la rouvrir — écarté. A1 est réalisable avec les briques déjà en place : le relais IA reste un simple proxy masquant la clé (URS-NF-044ter), la boucle d'appels et la lecture des stores restent 100 % client-side, cohérent avec l'architecture PWA sans backend. A3 est plus simple mais s'effondre dès qu'un cas réel dépasse la fenêtre de contexte ou nécessite une clarification humaine en cours de route (exactement le scénario §16/§38 de l'utilisateur : "il me manque une information, je la demande, j'attends la réponse, je continue") — A3 ne peut physiquement pas faire ça en un seul appel.
