...
1. `verifierBlocageExport.ts` — un seul statut de `Section` (`propose_par_ia_non_valide`) bloque l'export.
2. `gardesFinalisation.ts` (garde-fous U-01/U-02/U-03) — plusieurs règles de liens obligatoires, filtrées par point de contrôle (`entree_en_verification`/`cloture_valide_en_interne`).
3. `useContentPlanStore.gelerContentPlan` — `readiness !== 'pret'` bloque le gel d'un `ContentPlan`.
