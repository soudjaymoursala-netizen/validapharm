...
- **Non corrigé, documenté comme limite persistante**: `Diagram`/`DiagramNode`/`DiagramEdge` restent hors périmètre (non engagée — confirmé plutôt que contredit par cette lecture). Le N:M réel `ExtractionItem↔KnowledgeItem` reste simplifié en N:1 (`KnowledgeItem.extraction_item_id` unique).
