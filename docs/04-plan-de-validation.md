...
> ⚠️ **DOCUMENT À REVOIR (constaté le 21/08/2026 lors d'un audit de cohérence)** : ce Plan de validation v01 référence URS v01/AR v01/FS v01, aujourd'hui dépassées (URS en v20, catalogue de gabarits élargi à 15 familles, registre de risques passé de 14 à 58 entrées, **FS/FDS/SDS réécrites et auditées (v08/v10/v06)** — voir `03-specifications-fonctionnelles.md`). Sa structure (catégorisation GAMP 5, stratégie de vérification proportionnée au risque, étapes IQ/OQ/PQ) reste valide dans son principe, mais son contenu détaillé (§9 "13 gabarits", références R-xx) doit maintenant être mis à jour contre la conception à jour — la condition qui bloquait cette révision est levée. Ne pas utiliser pour piloter des tests réels en l'état.

## 1. Objet et portée

Définir la stratégie de qualification/validation de l'outil ValidaPharm lui-même (Phase 1 : version locale, mono-utilisateur), proportionnée au risque identifié dans l'analyse de risque AR-VALIDAPHARM-2026-001, avant toute mise en service pour la production de livrables "approuvés dans l'outil".

Cette Phase 1 vise un usage personnel ; le niveau de rigueur appliqué reste néanmoins celui d'une approche GAMP 5, pour permettre une extension directe (Phase 3) sans dette de validation à rattraper.
...
| OQ (Operational Qualification) | Vérifier que chaque fonction opère conformément à la FS sur sa plage prévue (gabarits, calculs, statuts, exports, routeur IA) | OQ-VALIDAPHARM-2026-001 |
...
## 5. Rôles et responsabilités (Phase 1, mono-utilisateur)

| Rôle | Responsabilité |
|---|---|
| Rédacteur / développeur | Conçoit, développe, exécute les tests, documente les résultats |
| Vérificateur | Revoit les protocoles et résultats avant approbation (peut être la même personne en Phase 1, avec limite reconnue et à lever en Phase 3 par une séparation réelle des rôles) |
| Approbateur | Décide de la mise en service de l'outil pour un usage "approuvé dans l'outil" |

**Limite reconnue de la Phase 1** : l'absence de séparation stricte rédacteur/vérificateur/approbateur (une seule personne physique) est un écart assumé, documenté ici, à lever explicitement en Phase 3 (multi-utilisateur) avant tout usage impliquant plusieurs parties prenantes ou un enjeu réglementaire opposable.
...
- [ ] Matrice de traçabilité complète (URS → FS → implémentation → test)
