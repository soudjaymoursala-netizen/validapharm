...
**Mixte assumée**, cohérente avec le vocabulaire déjà mixte des identifiants de domaine dans les documents de spécification et de conception (ex. `qualification_status`, `asset_node`):
- **Domaine métier** (fonctions/types qui implémentent une exigence fonctionnelle) : nommage **français** — ex. `calculerIPR`, `validerAbsenceDeCycle`, `presenterStatut`, `QualificationStatus`.
...
src/
  ...
  logique-metier/      Fonctions pures, testables indépendamment de l'UI
    ...
    resolution-conflit/    diff structuré sur conflit de SHA
  connecteurs/         GitHub (API), Drive (API), IA (ProviderAdapter), QMS (QMSConnectorAdapter) —
                       exclusivement des appels API HTTPS
