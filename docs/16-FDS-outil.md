...
| Impact Assessment / System Classification *(écran réellement construit v17, 29/08/2026 — le store existait depuis la Phase 3/v15 mais aucun composant ne l'exposait jusqu'ici, trouvé lors d'une simulation d'usage réelle)* | Méthode configurable par client, verdict Direct/Not Direct Impact | (nouveau v15) |
...
| Analyse de documents / Challenge de dossier | Chargement de document, constats affichés — **seule la moitié §4.8 (analyse structurelle, Phase 34) existe réellement, intégrée à Fiche Projet (voir ligne dédiée plus bas), pas comme écran séparé** ; (PID/certificat) non implémentés | |
...
| Journal d'anomalies *(ajouté v04 — audit QA spécialisés)* | **Écart v17 comblé v18 (31/08/2026)** : `JournalAnomalies.vue`, CRUD complet (Change Control/Déviation/CAPA/Investigation/Constat d'audit/Revue périodique), filtres, cross-référencement (Deviation→Investigation→CAPA), lien optionnel vers un `AssetNode`. **Le type de domaine `QualityEvent`, sa table Dexie et son store (`useQualityEventStore.ts`) existaient déjà en entier depuis la Phase 5** — la FDS v17 affirmait à tort qu'aucun type de domaine n'existait ; c'était un écart de traçabilité documentaire, pas un écart fonctionnel réel. Seul l'écran autonome manquait (`MissionWorkspace.vue` n'exposait ces événements que dans le contexte d'une Mission précise). | |
...
| Structure Système — extension Architecture Technique *(ajouté v16 — Phase 18)* | **Écart comblé v18 (31/08/2026)** : panneau "Relations techniques" intégré à `StructureSysteme.vue` — création/liste des `RelationTechnique` (`controle_par`/`connecte_a`/`heberge_sur`), traçage interactif de la chaîne technique (`chaineTechniqueDepuisNoeud`, déjà présent dans le store depuis la Phase 18 sans jamais avoir eu de consommateur UI). Reste consommée aussi par le Reasoning Engine (`tracer_chaine_technique`), sans changement. | |
...
| Bibliothèque de normes — analyse structurelle *(ajouté v16 — Phase 34, extension de l'écran Fiche Projet existant, pas un écran séparé)* | Détection déterministe et affichage d'une section "exigence URS" sans aucun lien vers une autre section du projet — jamais un verdict de conformité | (partiel) |
...
- **E5** : la séparation écran/export est garantie techniquement, pas seulement documentairement — aucun jeton de couleur/typographie de l'écran n'est importé par le moteur de génération d'export (détail SDS §7bis).
...
Chaque message est stocké comme ressource multilingue (fr/en/de dès Phase 1), jamais codé en dur dans une seule langue.
