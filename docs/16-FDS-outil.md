...
La spécification fonctionnelle décrit **quoi** et **comment fonctionnellement** l'outil doit fonctionner. Ce document descend un niveau : il décrit **l'expérience utilisateur et les algorithmes précis** — écrans, flux de navigation, machine à états complète, règles de validation champ par champ, formulation exacte des messages système — sans entrer dans l'implémentation technique (framework, base de données physique — relève de la spécification de conception technique, §9).

Toute règle déjà énoncée dans la spécification fonctionnelle n'est pas reformulée ici sauf si elle nécessite une précision de niveau conception (ex. la spécification fonctionnelle dit "un bandeau nomme le fournisseur actif" ; ce document précise le contenu exact et le déclenchement du bandeau).

## 2. Inventaire des écrans

| Écran | Rôle | Répond à |
|---|---|---|
| Tableau de bord / Vue portefeuille | Liste des projets, statuts agrégés, alertes | |
| Fiche Projet | Contexte, portée, sections liées; vue de traçabilité toujours backlog (tâche #12); section Documents *(réellement construite v20, 31/08/2026 — jusqu'ici cette ligne décrivait la cible sans écran réel, corrigé honnêtement plutôt que laissé comme faux positif)*: chargement de fichiers sous n'importe quel format, toujours étiquetés référence de travail (non maître), horodatés | |
| Éditeur de section | Rédaction guidée d'un livrable (gabarit) |.1bis |
| Panneau de suggestions | Idées de tests, sections manquantes — séparé du corps | |
| Panneau Chat expert | Conversation, sélection fournisseur, bandeau d'avertissement | |
| Bibliothèque de normes | Consultation/association de normes | |
| Assistant de stratégie de qualification | Méthode ACFC configurable par client, conclusion | |
| Impact Assessment / System Classification *(écran réellement construit v17, 29/08/2026 — le store existait depuis v15 mais aucun composant ne l'exposait jusqu'ici, trouvé lors d'une simulation d'usage réelle)* | Méthode configurable par client, verdict Direct/Not Direct Impact | (nouveau v15) |
| Computer System Assessment *(écran réellement construit v17, 29/08/2026 — même écart trouvé et comblé)* | Catégorie GAMP5 (fixe) + pertinence GxP/ERES | (nouveau v15) |
| Analyse de documents / Challenge de dossier | Chargement de document, constats affichés — **seule la moitié §4.8 (analyse structurelle) existe réellement, intégrée à Fiche Projet (voir ligne dédiée plus bas), pas comme écran séparé**; (PID/certificat) non implémentés | |
| Configuration client | Fournisseur IA, gabarit d'export, consentement télémétrie, qualification de fiabilité | `client_config` |
| Export | Choix de format, avertissements de blocage | |
| Journal d'anomalies *(ajouté v04 — audit QA spécialisés)* | **Écart v17 comblé v18 (31/08/2026)**: `JournalAnomalies.vue`, CRUD complet (Change Control/Déviation/CAPA/Investigation/Constat d'audit/Revue périodique), filtres, cross-référencement (Deviation→Investigation→CAPA), lien optionnel vers un `AssetNode`. **Le type de domaine `QualityEvent`, sa table Dexie et son store (`useQualityEventStore.ts`) existaient déjà en entier** — la version précédente de ce document affirmait à tort qu'aucun type de domaine n'existait ; c'était un écart de traçabilité documentaire, pas un écart fonctionnel réel. Seul l'écran autonome manquait (`MissionWorkspace.vue` n'exposait ces événements que dans le contexte d'une Mission précise). | |
| Configuration des connecteurs QMS *(ajouté v05)* | **Écart v17 comblé v18 (31/08/2026)**: `ConfigurationConnecteursQMS.vue` + `useConnecteursQMSStore.ts`, CRUD de configuration (nom/type/actif/paramètres de connexion) pour les 6 types de `Connector`. **Périmètre assumé**: aucun test de connexion réel pour `veeva_vault`/`sharepoint`/`dossier_reseau`/`edms_generique` (adaptateurs non implémentés, déjà documenté ainsi dans le type de domaine) — `github`/`google_drive` ne font que consigner une configuration, sans dupliquer les adaptateurs de stockage déjà testés ailleurs (`ConfigurationClient.vue`/`ConfigurationDrive.vue`). | |
| Structure Système *(ajouté v05)* | Arbre + graphe des `asset_node` d'un client, création/édition de nœud, vue graphique, **statut de qualification et périodicité de requalification éditables manuellement (ajouté v18, 31/08/2026, jamais de transition automatique fabriquée, badge visuel non persistant si échéance dépassée)** | |
| Dossier vivant d'un actif *(ajouté v05)* | **Écart v17 partiellement comblé v18, complété v19 (31/08/2026)**: `DossierVivantActif.vue`, agrégation en lecture seule de tout ce qui porte un `assetNodeId`/`asset_node_id` explicite (ACFC, Impact Assessment, CSV Assessment, Risk Assessment/AMDEC, Missions ancrées, **Journal d'anomalies — ajouté v19, trouvé manquant en consultant l'écran juste après avoir consigné un constat d'audit réel**) + chaîne technique + statut de qualification. **Périmètre non couvert, documenté honnêtement à l'écran plutôt que simulé**: aucune section de gabarit de projet (DQ/FAT/SAT/IQ/OQ/PQ…) n'agrégée — le modèle de données actuel ne porte aucun lien direct section↔nœud (seul le lien section↔section existe), contrairement à ce que décrivait la spécification fonctionnelle d'origine. | |
| Blocage d'incompatibilité de données *(ajouté v12)* | Écran plein affiché avant tout chargement si `schema_version` des données est postérieur à la version de l'application (message U-12) | |
| Coquille applicative — sidebar/Accueil *(ajouté v16, dans le cadre de la convergence architecturale, `docs/convergence/PHASE_16_COQUILLE_UX_SPEC.md`; différenciation Mode Expert/Assistant réellement comblée v21, 31/08/2026)* | `BarreLaterale`/`CoquilleApplication`/`AccueilQueVoulezVousFaire`: bascule Mode Expert/Mode Assistant — le Mode Assistant restreint désormais réellement la navigation "Mon site" au parcours guidé (Missions, Structure Système, Stratégie de qualification, évaluations, Procédures, Assistant IA) et masque les écrans de configuration avancée, jamais un écran supprimé; mémorisation du client actif côté navigateur, entrée "Accueil" (Que voulez-vous faire ?) | |
| Missions *(ajouté v16, `docs/convergence/PHASE_17_MISSION_WORKSPACE_SPEC.md`)* | `ListeMissions.vue` (liste + création de `Mission` par client) et `MissionWorkspace.vue` (Activités avec dépendances, association `QualityEvent`, assemblage/affichage d'un `ContextSnapshot`, invocation du Reasoning Engine scopée à la Mission, badge de confiance, historique des invocations) | |
| Structure Système — extension Architecture Technique *(ajouté v16)* | **Écart comblé v18 (31/08/2026)**: panneau "Relations techniques" intégré à `StructureSysteme.vue` — création/liste des `RelationTechnique` (`controle_par`/`connecte_a`/`heberge_sur`), traçage interactif de la chaîne technique (`chaineTechniqueDepuisNoeud`, déjà présent dans le store sans jamais avoir eu de consommateur UI). Reste consommée aussi par le Reasoning Engine (`tracer_chaine_technique`), sans changement. | |
| Procédures *(ajouté v16)* | `RevueStructureProcedure.vue` (route `/clients/:clientId/procedures`): import `.docx`/`.pdf` ou collage de texte, génération d'une proposition de structure (parseur déterministe en premier, repli IA uniquement si celui-ci ne retourne strictement rien), revue éditable des sections/étapes proposées (case "Retenir" par étape, badge de confiance IA), formulaire de métadonnées, confirmation — seule la confirmation écrit réellement `Procedure`/`ProcedureStep` | |
| Éditeur de section — génération de brouillon par adaptation *(ajouté v16, panneau intégré à l'Éditeur de section, pas un écran séparé)* | Coller un texte ou uploader un fichier de référence (`.docx`/`.pdf`), confirmation de droit d'usage obligatoire, génération limitée aux champs scalaires du gabarit, checklist de relecture par `DefinitionSection` gate le bouton de validation, badge sur les champs générés | |
| Éditeur de section — gabarit d'export personnalisé *(ajouté v16, panneau intégré à l'Éditeur de section)* | Import d'un gabarit `.docx` client, refus explicite à l'import si les balises obligatoires (`redacteurs`/`approbateur_final`/`historique_revisions`) ne sont pas mappées, sélection du gabarit actif pour l'export | |
| Éditeur de section — liens vers d'autres sections *(ajouté v16 — correctif du 28/08/2026, non versionné en phase de convergence numérotée)* | Panneau permettant de choisir une autre section du même projet et de la lier/délier (lien non dirigé, idempotent, journalisé) — seule interface permettant de satisfaire légitimement les garde-fous U-01/U-02/U-03 (§3.3), auparavant contournables uniquement via "Forcer" | |
| Panneau Chat expert — mode audit simulé *(ajouté v16, sous-mode du panneau Chat expert existant, pas un écran séparé)* | Bascule explicite `chat_normatif`/`audit_simule`, sélection de persona(s) d'auditeur simulé (Swissmedic/FDA/cabinet de conseil GxP/QA spécialisée), bandeau de rappel à chaque activation, qualification de fiabilité indexée par mode | |
| Bibliothèque de normes — analyse structurelle *(ajouté v16, extension de l'écran Fiche Projet existant, pas un écran séparé)* | Détection déterministe et affichage d'une section "exigence URS" sans aucun lien vers une autre section du projet — jamais un verdict de conformité | (partiel) |

**Note méthodologique (ajoutée v16, mise à jour v18)**: contrairement aux écrans ci-dessus, les capacités suivantes n'ont toujours aucun écran, par choix documenté à chaque phase (aucun consommateur UI réel identifié au moment de leur construction): `Source`/`Extraction`/`KnowledgeItem`, `ContentPlan`, `RiskAssessment` (AMDEC autonome — store `useRiskAssessmentStore.ts` existant, aucun écran dédié, hors périmètre de ce rattrapage v18), Knowledge Graph (`KnowledgeRelation`). `QualityEvent` a désormais un écran (`JournalAnomalies.vue`, v18) et n'appartient plus à cette liste. Ne pas chercher les autres dans l'inventaire ci-dessus — elles sont consommées uniquement par le Reasoning Engine ou par test unitaire. Voir §12 pour la liste consolidée des écarts documentaires ainsi comblés et de ceux qui restent ouverts.

## 2bis. Charte graphique et identité visuelle *(ajouté v11)*
...
| Texte principal | `#14152A` (clair) / `#F1F2F8` (sombre) | Corps de texte, titres — taille minimale équivalente 14px pour le corps *(ajouté v11 — revue-charte, E4)* |
| Texte secondaire | `#5C5F76` (clair) / `#A3A6BD` (sombre) | Libellés, méta-informations |
| Marque (indigo, réglée à la main) | `#4638C2` (clair) / `#8B7FFF` (sombre) | Actions primaires, liens, éléments actifs — contraste texte/lien sur fond carte clair: 8.06:1 |
...
**Palette sémantique — `qualification_status` (mitige R-59)**
...
- **E5**: la séparation écran/export est garantie techniquement, pas seulement documentairement — aucun jeton de couleur/typographie de l'écran n'est importé par le moteur de génération d'export (détail).
...
- La sidebar (barre latérale complète, avec tous les liens vers des routes protégées) s'affichait sur l'écran de connexion lui-même, avant toute authentification — chaque lien aurait immédiatement renvoyé vers `/connexion` (garde de routeur), et la première impression du produit était brouillée par une navigation sans objet. Corrigé: `CoquilleApplication.vue` masque désormais la sidebar sur la route `connexion`.
...
Tableau de bord
 ...
 à la création uniquement (ajouté v02 — revue E5) — une fois la section
 créée, `section.language` est fixe (changer de langue en cours de rédaction
 équivaudrait à une re-rédaction, hors périmètre)
...
**Chemin de rejet (ajouté v02 — revue E1)**: à tout moment durant `en_verification` ou `en_approbation`, un relecteur ou l'approbateur final peut rejeter la section — transition unique retour à `brouillon_aide`, avec **motif obligatoire** capturé dans `revisions` (`motif` déjà prévu dans le modèle, jusqu'ici jamais rendu obligatoire au niveau conception). Aucun statut "rejeté" distinct n'est introduit — le rejet ramène directement en rédaction, cohérent avec la priorité déjà retenue: fiabilité du contenu avant automatisation du workflow.
...
**Justification de l'asymétrie des points 1-3 (ajoutée v02 — revue E6)**: les blocages Contexte procédé (point 1) et Métrologie (point 2) se déclenchent à la **finalisation** (entrée en `en_verification`), tandis que le blocage Maintenance (point 3) se déclenche à la **clôture** (passage à `valide_en_interne`). Ce n'est pas une incohérence: c'est un choix délibéré, aligné sur le texte qui reprend explicitement Annexe 15 §3.12 (le plan de maintenance préventive est un livrable de clôture d'OQ, pas un prérequis de démarrage de sa vérification — contrairement au contexte procédé, nécessaire dès la conception des tests).

Chaque blocage propose un bouton "Forcer" avec **saisie d'un motif obligatoire** (ajouté v02 — revue E2), journalisé aux côtés de l'horodatage/acteur déjà prévus (§4.3bis) — sauf le blocage export IA-non-validé qui utilise son propre mécanisme de forçage déjà décrit plus haut, désormais soumis à la même règle de motif obligatoire (voir §7, U-04).
...
### 3.6 Résolution de conflit de synchronisation (ajouté v02 — revue E5; re-affiné v14 — architecture web pure)
...
### 3.7 Journal d'anomalies (ajouté v04 — audit QA spécialisés, MAJ-01)
...
Volontairement léger — pas un processus CAPA complet (celui-ci reste réservé au cas spécifique du moteur de calcul, §7 U-07): une simple liste consultable, proportionnée à un usage mono-utilisateur.

### 3.8 Connecteurs QMS tiers — flux pull/push (ajouté v05)
...
Pull (depuis l'éditeur de section ou la fiche Projet)
 ...
 → Chaque champ importé prend le statut "proposé — non validé" (même traitement que §4.1bis)
...
Push (depuis une section valide_en_interne)
 ...
 transmis (référence, titre, statut, version) — ajouté v07, revue v06, E3
...
### 3.9 Structure Système — création, dossier vivant, statut (ajouté v05)

```
Configuration de la hiérarchie (ajouté v06 — revue v05, E5, préalable à tout usage)
...
Reparentage d'un nœud existant (ajouté v07, revue v06, E5)
 → [Modifier le parent] → nouvelle vérification anti-cycle, journalisation, instantanés
 des livrables déjà liés non affectés
 → **(Clarifié v06 — revue v05, E5)** Le dossier vivant repose sur les liens directs
...
### 3.10 Mission workspace — assemblage de contexte et raisonnement (ajouté v16)
...
Missions (par client)
 ...
 (généralisés, jusqu'ici câblés sur le seul store Structure Système)
 → narratif OÙ (AssetNode)/QUOI (ManufacturingContext)/COMMENT (Procedure)/
 POURQUOI-IMPACT (QualityEvent), injecté dans le prompt du
 ...
 → réponse avec état de confiance (connu/inféré/inconnu/conflit/
...
**Garde-fou non négociable**: aucune section Assessment/Requirement/Test/Evidence n'est rattachée à la Mission dans cet incrément (appartiendrait à `Strategy`, non construite); aucun indicateur de "Validation State" n'est calculé ou affiché sur la Mission (vue calculée diagnostique différée, jamais un remplacement automatique d'un statut saisi manuellement).

### 3.11 Repli IA-assisté de structuration procédurale (ajouté v16)
...
Procédures (par client)
 ...
 — zéro appel réseau
 2. Repli IA-assisté uniquement si le parseur déterministe ne retourne
...
| U-07 | Défaut moteur corrigé | "{n} section(s) validées ont été produites avec la version {x} du moteur, aujourd'hui corrigée. Revue d'impact requise. [Voir la liste] [J'ai pris connaissance]" — **(v02 — revue E3)** modale bloquante (pas un bandeau ignorable): le bouton "J'ai pris connaissance" est le seul moyen de la masquer, action journalisée avec horodatage |
...
| U-12 *(v12 — résorption de dette)* | Démarrage de l'application sur des données dont `schema_version` est postérieur à ce que cette version sait lire | Écran de blocage au démarrage (pas une simple modale sur l'écran principal, l'application ne doit pas exposer un état incohérent): "Cette version de ValidaPharm ne peut pas ouvrir ces données — elles ont été créées ou migrées avec une version plus récente. Mettez à jour l'application avant de continuer, ou revenez à la version {x} pour les rouvrir." Aucune écriture n'a lieu avant confirmation de compatibilité. |

Chaque message est stocké comme ressource multilingue (fr/en/de), jamais codé en dur dans une seule langue.

## 8. Accessibilité et multilingue — détail de conception

- Navigation clavier: ordre de tabulation logique par écran (haut→bas, gauche→droite), raccourcis `Ctrl+S` (sauvegarde manuelle immédiate) et `Échap` (fermeture de modale) sur toutes les vues.
- RTL/CJK: la mise en page utilise des propriétés logiques (`start`/`end` plutôt que `left`/`right`), même si seuls FR/EN/DE sont livrés — pour ne pas nécessiter de refonte de layout à l'ajout de l'arabe/chinois.
- **Lecteur d'écran (ajouté v12 — résorption de dette)**: tout composant interactif (bouton, champ, lien) porte un nom accessible explicite (jamais une icône seule sans libellé programmatique); les badges de statut de qualification exposent leur libellé texte complet, pas seulement leur icône visuelle; toute confirmation/erreur affichée via les messages §7 est annoncée (zone dynamique de type live region) au moment de son apparition, pas seulement visible; navigation par landmarks (zone principale, navigation, panneau latéral) sur les écrans à plusieurs zones (ex. Éditeur de section + Panneau de suggestions). Vérification par parcours manuel au lecteur d'écran standard (NVDA/VoiceOver) sur les parcours critiques (navigation, saisie, export) — choix délibéré de test exploratoire plutôt qu'automatisé, proportionné à la portée actuelle.
- **Performance perçue (ajouté v12 — résorption de dette)**: chargement du tableau de bord et ouverture d'une section restent sous 2 secondes perçues dans le volume de référence (500 projets/5000 sections); au-delà, chargement progressif ou pagination de la liste plutôt qu'un blocage — cohérent avec le principe "retour visuel immédiat" déjà posé en §2bis (aucune action sans feedback, même quand elle prend du temps).

## 8bis. Principe directeur pour la conception technique (ajouté v04 — audit cabinet de conseil GxP, MAJ-01)

**Séparation logique métier / présentation, non négociable pour la conception technique** : la logique testable de l'outil — moteur de calcul (§5), machine à états (§3.2), grille de décision de qualification, détection de liens manquants — DOIT être conçue comme un ensemble de fonctions/modules isolés, **testables indépendamment de toute interface graphique**. Aucun calcul ni transition d'état ne doit être écrit uniquement à l'intérieur d'un composant d'affichage. Ce principe n'est pas nouveau dans son intention (il découle directement de l'exigence de "tests unitaires dédiés"), mais devait être rendu explicite comme contrainte architecturale avant la rédaction de la spécification de conception technique, pour ne pas devoir la corriger après coup.

## 9. Suite de la cascade documentaire

Ce document sert d'entrée à la **spécification de conception technique** (architecture technique : choix de framework, schéma de cache local, contrats d'API précis avec GitHub/Drive/fournisseurs IA, mécanisme de résolution de conflit par détection SHA). Aucun HDS (pas de matériel dédié) ni de Data Migration Plan (dépôt Git vierge, décision du 22/08/2026).

## 10. Matrice de traçabilité (spécification fonctionnelle → conception fonctionnelle)
...
| (journal d'anomalies) | §2, §3.7 |
| (connecteurs QMS) | §2, §3.8, §7 (U-11) |
| (Structure Système) | §2, §3.9, §7 (U-09/U-10) |
| (charte graphique) | §2bis |
| (perf/lecteur d'écran/rollback) | §8, §7 (U-12) |
| §3.10 Mission workspace | |
| §3.11 Repli structuration procédurale | |
| §3.12 Correctif tableau_dynamique | (correctif, aucune exigence nouvelle) |
| §5 AMDEC autonome/readiness/Compliance Engine/Knowledge Graph | |
...
Ce document a été rattrapé le 28/08/2026 après un écart de traçabilité constaté : de nombreux domaines ajoutés en cours de convergence architecturale avaient été committés, testés et intégrés à la spécification fonctionnelle sans jamais être redescendus dans ce document ni dans la spécification de conception technique — un écart de traçabilité GAMP5 non déclaré, trouvé par audit croisé des documents entre eux. Le présent rattrapage (§2, §3.10 à §3.12, §5) comble le contenu fonctionnel manquant mais **ne reproduit pas le niveau de détail complet de chaque spec de conception individuelle** (`docs/convergence/PHASE_13_*` à `PHASE_34_*`) — celles-ci restent la référence pour le détail exhaustif de chaque mécanisme.
...
- **Dossier vivant d'un actif**: comblé pour les évaluations/missions/relations techniques rattachées à un `AssetNode`, mais **pas pour les sections de gabarit de projet** (DQ/FAT/SAT/IQ/OQ/PQ…) — aucun lien direct section↔nœud n'existe dans le modèle de données actuel (seul le lien section↔section existe), contrairement à ce que décrivait la spécification fonctionnelle d'origine. Corrigé honnêtement dans l'écran (§2) plutôt que simulé.
