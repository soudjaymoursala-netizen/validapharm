...
**Note de cohérence (24/08/2026, ajoutée en fin de journée)** : l'utilisateur a fourni un "prompt maître d'audit et de montée en maturité" (méthodologie GxP/IA exigeante, zéro complaisance) — conservé verbatim en `docs/audit/PROMPT-MAITRE-AUDIT-MATURITE.md`, décision d'application documentée en **§6quater** ci-dessous. À lire par toute session future avant de reproposer un audit complet : la décision d'application progressive est déjà tranchée.

**Note de cohérence (24/08/2026)** : suite à deux suggestions relayées par l'utilisateur d'un ingénieur logiciel externe consulté sur le projet, l'URS passe en **v24** (mode audit simulé dans le chat expert — URS-F-038 à 040 ; modularité et protection de la propriété intellectuelle — URS-NF-056 à 058) et l'analyse de risque en **v25** (R-64 dépendance réseau du relais IA, R-65 dérive de coût IA, R-66 faux sentiment de protection du code). Point de vigilance explicitement communiqué à l'utilisateur : la modularité du code protège la maintenabilité, pas la confidentialité du code source d'une PWA (techniquement extractible par construction) — la protection réelle en cas de livraison à un tiers est contractuelle (licence/NDA), hors périmètre de conception logicielle. Le choix du moteur IA de production (fournisseur, modèle, relais serverless de masquage de clé) reste **à trancher** ; il nécessitera un test de joignabilité réseau du domaine retenu depuis le poste de travail de l'utilisateur, selon la même méthode que pour `api.github.com`/`*.github.io` (R-62, clos).

**Note de cohérence (26/08/2026)** : l'utilisateur a clarifié explicitement la vision produit finale — un environnement de travail qualité qui reconstruit le contexte d'un cas réel, raisonne (impact→risque→requirement→test manquant) et dit explicitement "je ne sais pas" plutôt qu'une collection de modules ACFC/Risk/CSV/IQ/OQ. Vérifié comme cohérent avec le package source Drive dès l'origine (domaines "Work" et "AI" du modèle cible, jamais engagés en code jusqu'ici), pas une nouvelle direction. Revue panel E1-E7 dédiée + 3 décisions actées (TD-007 à TD-009, `docs/convergence/TECHNICAL_DECISIONS.md`), plan révisé en Phases 13-17 (`docs/convergence/CONVERGENCE_PLAN.md`) — voir `CONTEXTE-REPRISE-SESSION.md` §5.12 pour le récit complet. Aucun code de ces phases écrit à ce stade.
...
## 6quater. Prompt maître d'audit et de montée en maturité GxP/IA (méthodologie ajoutée le 24/08/2026)

**Source** : l'utilisateur a fourni le 24/08/2026 un document long ("VALIDAPHARM — MASTER AUDIT & MATURITY IMPROVEMENT PROMPT"), conservé **verbatim** dans `docs/audit/PROMPT-MAITRE-AUDIT-MATURITE.md` — ne pas paraphraser cette source, s'y référer directement. Il définit une organisation virtuelle de 15 équipes d'audit (Product, Architecture, Software Engineering, AI Engineering, AI Evaluation, Cybersécurité, QA, GxP/CSV/CSA, Data Integrity, Validation, DevOps, Documentation, Independent Reviewer), avec une mentalité "zéro complaisance" (aucune décision antérieure n'est immunisée contre la critique), "montrez-moi la preuve" façon inspecteur GxP, "no fake evidence"/"no cosmetic compliance", et un cycle Discovery → Multi-team audit → Gap Assessment → Remediation Plan → Implementation → Independent Inspection → Rapport de maturité final (`docs/audit/GAP_ASSESSMENT.md`, `REMEDIATION_PLAN.md`, `VALIDAPHARM_MATURITY_ASSESSMENT.md`).

**Constat fait le jour même, avant d'appliquer quoi que ce soit** : ce prompt suppose un logiciel **déjà mature, déployé, avec IA et sécurité en production, utilisateurs réels** — ce n'est pas l'état de ValidaPharm le 24/08/2026. À cette date : documentation de conception mature et déjà passée par de nombreuses revues (URS v25/AR v27/FS/FDS/SDS), mais **code démarré ce jour-là même** (premier incrément vertical : modèle de données, cycle de vie des sections, persistance, 4 écrans, connecteur GitHub — tous écrits et testés le 24/08/2026). Zéro ligne de code IA, zéro infrastructure déployée, zéro authentification (mono-utilisateur Phase 1 assumé, cadrage §4). Faire tourner les 15 équipes à la lettre maintenant produirait surtout des lignes "non applicable — pas encore construit", pas un audit utile.

**Décision explicite de l'utilisateur (24/08/2026, tranchée après une question de clarification)** — à appliquer sans avoir à la redemander dans une session future :
1. **Pas d'audit formel complet maintenant.** Les livrables `docs/audit/GAP_ASSESSMENT.md` / `REMEDIATION_PLAN.md` / `VALIDAPHARM_MATURITY_ASSESSMENT.md` (Phases 1 à 7 du prompt) sont **reportés à la fin** — quand il y aura une IA, une sécurité et une infrastructure réelles à auditer, pas avant.
2. **La mentalité du prompt s'applique dès maintenant, en continu**, comme discipline de travail normale — pas comme événement ponctuel différé : zéro complaisance, "montrez-moi la preuve", proactivité sur les gaps trouvés, jamais de fausse preuve/conformité cosmétique. Cohérent avec et renforce la méthodologie déjà en place (§6bis, §6ter) plutôt que de la remplacer.
3. **Dès qu'une équipe du prompt devient concrètement pertinente** pour ce qui est en cours de construction (ex. Architecture/Software Engineering/Data Integrity/GxP-Audit Trail dès qu'il y a du code ; AI Engineering/AI Evaluation/Cybersécurité-production/Validation seulement quand il y aura de l'IA/une infra/un dossier de validation), appliquer son regard **en temps réel**, dans le cycle normal de construction — pas en fin de compte.
4. **Revenir en arrière sur ce qui est déjà construit et corriger réellement les gaps trouvés**, immédiatement, pas seulement les documenter pour plus tard. Première application le 24/08/2026 : relecture critique du premier incrément (regards Architecture/Data Integrity/GxP Audit Trail) → deux gaps réels trouvés et corrigés le jour même (identité "acteur" incohérente/codée en dur entre deux fichiers ; sauvegarde de contenu sans entrée `audit_log` malgré FS §3) — détail dans l'historique Git, pas dupliqué ici.

**Pour toute session future** : ce prompt existe, cette décision d'application progressive est déjà tranchée — ne pas la re-proposer ni la re-débattre sans raison nouvelle. Le déclenchement explicite "Audit Validapharm as an FDA/Swissmedic inspector" (prompt §19) reste disponible à la demande de l'utilisateur à tout moment, indépendamment du calendrier de l'audit complet.
