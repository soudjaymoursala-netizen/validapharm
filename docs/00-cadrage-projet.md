...
Ce document n'est pas encore poussé sur GitHub (décision du 21/08/2026) : il vit en local le temps de finaliser le cadrage.

**Note de cohérence (21/08/2026)** : les décisions structurantes de ce document restent valides. Pour le détail exhaustif et à jour des exigences (catalogue des gabarits, garde-fous IA, architecture multi-fournisseurs, multilingue, etc.), se référer à `01-URS-outil.md` (v20 au 22/08/2026) qui fait foi en cas de divergence de détail avec ce document de cadrage. **La cascade de conception est désormais complète et intégralement à jour** : FS (v08), FDS (v10), SDS (v06) couvrent l'intégralité de l'URS v20, y compris les cinq besoins ajoutés après la clôture initiale de la cascade (connecteurs QMS tiers §4.9, Structure Système §4.10 — référentiel d'actifs, dossier vivant, export PDF historique, statut de qualification), chacun ayant traversé panel multi-experts + audits Swissmedic/FDA (et cabinet GxP/QA spécialisés pour la FDS). L'exclusion "Intégration ERP/QMS tiers" du §8 URS a été levée le 22/08/2026 sur décision explicite de l'utilisateur, malgré le coût de réouverture signalé à chaque fois. **Dépôt dédié créé le 22/08/2026** (`/home/user/validapharm`, local, non poussé — voir §"Stockage" ci-dessous) : documentation de conception (docs/) + environnement de conception (structure `/data`, driver de fusion applicatif JSON, hook de scan de secrets, squelette de portail qualité CI) initialisés en un commit racine. Ce dépôt `app-comores-transport/validapharm/docs/` reste la copie de travail historique de cette session ; le dépôt `/home/user/validapharm` fait désormais foi comme source de vérité pour la conception. **La conception (code) démarre le 23/08/2026.** Les documents de qualification de l'outil lui-même (VMP + protocoles IQ/OQ/PQ) seront réécrits contre cette conception à jour **après** la conception, sur décision explicite de l'utilisateur du 22/08/2026 (registre de risques : 58 entrées).
