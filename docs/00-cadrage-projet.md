...
**Note de cohérence (23/08/2026)** : les décisions structurantes de ce document restent valides. Pour le détail exhaustif et à jour des exigences (catalogue des gabarits, garde-fous IA, architecture multi-fournisseurs, multilingue, etc.), se référer à `01-URS-outil.md` (v21 au 23/08/2026) qui fait foi en cas de divergence de détail avec ce document de cadrage. **La cascade de conception est désormais complète et intégralement à jour** : FS (v09), FDS (v11), SDS (v08) couvrent l'intégralité de l'URS v21, y compris les cinq besoins ajoutés après la clôture initiale de la cascade (connecteurs QMS tiers §4.9, Structure Système §4.10 — référentiel d'actifs, dossier vivant, export PDF historique, statut de qualification) et la charte graphique/identité visuelle ajoutée le 23/08/2026 via la checklist de complétude par domaine (§6ter), chacun ayant traversé panel multi-experts + audit(s) calibré(s) au sujet traité (Swissmedic/FDA/cabinet GxP/QA spécialisés pour le cœur fonctionnel, audit accessibilité dédié pour la charte graphique). L'exclusion "Intégration ERP/QMS tiers" du §8 URS a été levée le 22/08/2026 sur décision explicite de l'utilisateur, malgré le coût de réouverture signalé à chaque fois. **Dépôt dédié créé le 22/08/2026, poussé sur GitHub le 23/08/2026** (`soudjaymoursala-netizen/validapharm`, `/home/user/validapharm` en local) : documentation de conception (`docs/`, réorganisée en documents vivants + `docs/archive/`) + environnement de conception (structure `/data`, driver de fusion applicatif JSON, hook de scan de secrets, squelette de portail qualité CI) + prototype initial rapatrié (`prototype-initial/`). Ce dépôt fait foi comme source de vérité pour la conception ; `app-comores-transport` ne contient plus aucun contenu ValidaPharm. **La conception (code) démarre le 23/08/2026.** Les documents de qualification de l'outil lui-même (VMP + protocoles IQ/OQ/PQ) seront réécrits contre cette conception à jour **après** la conception, sur décision explicite de l'utilisateur du 22/08/2026 (registre de risques : 59 entrées).
...
| Performance & capacité | temps de réponse, volumétrie, dégradation | **Dette explicitement acceptée (23/08/2026)** — pas de cible chiffrée à ce jour (URS-NF-052 pose le principe "réactif", sans seuil précis). Décision : traité à l'ouverture de la SDS d'implémentation détaillée (une fois le framework choisi, les seuils réalistes dépendent de lui), pas avant. Ne bloque pas le démarrage de la conception. |
| Internationalisation | langues, formats date/nombre/devise, RTL | Couvert (URS-NF-040 et suivants, FDS §8) |
| Accessibilité | contraste, clavier, lecteurs d'écran | **Couvert pour le contraste des statuts** (URS-NF-054quater, audit accessibilité dédié) ; clavier couvert (URS-NF-050) ; **dette explicitement acceptée** sur lecteur d'écran (aucune exigence ARIA/screen-reader) — faible priorité Phase 1 mono-utilisateur, à revisiter si l'usage s'élargit |
| Conformité réglementaire | GAMP5, ALCOA+, Part 11, predicate rules | Couvert (cœur du projet) |
| Opérations & support | sauvegarde, restauration, montée de version, désinstallation | Sauvegarde/migration couvertes (SDS §3), miroir Drive contractualisé (SDS §5bis, ajouté 23/08/2026 — asymétrie trouvée en revue de cohérence). **Dette explicitement acceptée** sur désinstallation/rollback applicatif — faible priorité Phase 1 local-first (désinstallation = suppression du dossier local, pas de mécanisme dédié nécessaire), à revisiter en Phase 3 |
