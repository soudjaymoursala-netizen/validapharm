...
- **Portail de qualité technique (ajouté v02 — revue SDS, E4, mitige AR-R-46)** : la maîtrise des changements n'est pas une pratique déclarée mais **appliquée automatiquement** — un pipeline d'intégration continue exécute la suite de tests unitaires de la Couche Logique métier à chaque proposition de modification, et **bloque techniquement** toute fusion vers la branche principale protégée tant qu'un test échoue. Aucune procédure de contournement manuel de ce blocage n'est prévue en Phase 1.
...
- Interface (`GitHubConnector`) : `lire(chemin): { contenu, sha }`, `ecrire(chemin, contenu, shaAttendu): Confirmation` — chaque écriture transmet le `sha` lu au moment de la dernière lecture (concurrence optimiste). Écritures sur la branche principale protégée (AR-R-10).
- **Attribution** : chaque commit créé via l'API est attribué à l'utilisateur authentifié par son jeton — attribution fiable mais **pas une signature cryptographique GPG/SSH locale**, limite Phase 1 assumée et documentée (ne jamais présenter ces commits comme "signés" dans l'UI ou l'export).
...
- Interface (`DriveConnector`) : `miroir(snapshotGit): Confirmation` — un seul point d'entrée, pas de logique métier propre. Le connecteur Drive **n'est jamais une source de vérité, jamais lu par l'application** (§3 FS "toute divergence se résout en faveur de Git").
...
- Store `useQualityEventStore.ts` (Phase 5) : `type` ∈ `change_control | deviation | capa | investigation | audit_finding | periodic_review`, `statut` ∈ `ouvert | en_cours | cloture`, référencement optionnel entre deux événements (ex. Deviation → Investigation → CAPA), garde-fou DEC-002/DEC-055 : un `QualityEvent` externe (`reference_externe`) n'est jamais un verrou sur un autre module.
- Écran Journal d'anomalies (FDS §3.7, `JournalAnomalies.vue`, ajouté v18) : CRUD complet + filtres par type/statut, lien optionnel vers un `AssetNode`.

## 7bis. Charte graphique — implémentation technique (ajouté v07, FDS §2bis)
...
- Volume de référence Phase 1 : 500 projets / 5000 sections par client. Les listes (tableau de bord, inventaire d'un référentiel Structure Système, dossier vivant) sont **paginées ou virtualisées** au-delà d'un seuil d'affichage (ex. 100 lignes) — jamais un rendu intégral de la liste complète en mémoire du navigateur.
...
| Accessibilité lecteur d'écran | Exploratoire (parcours manuel NVDA/VoiceOver) | Parcours critiques uniquement (navigation, saisie, export) — choix délibéré, proportionné à la Phase 1 |
...
| FS §4.19-4.23 (Work/Context/AI/Coquille UX/Mission workspace) | §3bis, §6ter, §6quater |
| FS §4.18 (Organization/Workspace) | §3bis, §6quinquies |
| FS §4.15 (Knowledge Graph) | §3bis, §6sexies |
| FS §4.24 (Architecture Technique) | §3bis (table `relationsTechniques`), §6sexies — écran comblé v18 (voir FDS §2) |
...
**v18 (31/08/2026)** : relations techniques `AssetNode` comblées — panneau intégré à `StructureSysteme.vue`, réutilisant `creerRelationTechnique`/`chaineTechniqueDepuisNoeud` déjà présents dans `useStructureSystemeStore.ts` depuis la Phase 18 (seule l'UI manquait). Le Journal d'anomalies (§8) est corrigé pour refléter l'architecture Dexie réelle plutôt qu'un texte pré-bascule jamais mis à jour, et s'appuie sur `QualityEvent`/`useQualityEventStore.ts` (Phase 5), existant déjà en entier avant v18.
...
- `RiskAssessment` (AMDEC autonome, Phase 29) : store complet (`useRiskAssessmentStore.ts`) sans aucun écran dédié — trouvé lors de l'inventaire v18 (FDS §12), laissé hors périmètre de ce rattrapage.

---
*Document vivant, version 18 (31/08/2026) : relations techniques `AssetNode` comblées (§11), Journal d'anomalies (§8) corrigé pour refléter l'architecture Dexie réelle, §12 mis à jour. v16 (28/08/2026) : rattrapage documentaire Phases 12-34 de convergence architecturale — §3bis (schéma de données), §6ter (Reasoning Engine), §6quater (Context Engine), §6quinquies (Organization/Workspace), §6sexies (Knowledge Graph), §12 (écarts connus, dont ceux qui restent ouverts). Trouvé et comblé suite à un audit croisé FS/FDS/SDS du 28/08/2026 ayant constaté que ce document était figé vers la Phase 11-12 malgré une FS à jour jusqu'à la Phase 34. Historique v02-v10 : voir corps du document. v11 (23/08/2026) : architecture web pure sans installation. v12 (23-24/08/2026) : architecture détaillée — stratégie Git Trees API pour éviter l'épuisement du quota GitHub (5000 req/h) ; hébergement GitHub Pages confirmé après vérification réseau réelle (AR-R-62 clos) ; bibliothèques complémentaires actées (Pinia, Vue Router, Dexie.js, vite-plugin-pwa). v13 (24/08/2026) : relais IA de production — conception du relais (Cloudflare Workers, sans état, §10quater), routage de modèle par mode d'usage, plafond de dépense à deux niveaux. v14 (24/08/2026) : test de joignabilité réseau du relais vérifié et clos (AR-R-64). v15 (26/08/2026) : déploiement GitHub Pages effectivement construit (§10bis passe de décision documentée à mécanisme réel).*
