...
- **Code réel depuis le 07/09/2026** : `workers/ia-relay/` — architecture swappable, testée (voir `22-SDS-outil.md` §10quater et `workers/ia-relay/README.md`). **Fournisseur câblé : ChatGPT (OpenAI)**, choix explicite de l'utilisateur qui remplace le "Claude par défaut" ci-dessus (décision de conception initiale, jamais un fait figé) — l'implémentation Claude/Anthropic reste dans le dépôt, testée, pour un retour en arrière sans reconstruction. **Déployé et vérifié en conditions réelles le 07/09/2026** : Worker `validapharm-ia-relay` actif sur Cloudflare (connecté au dépôt GitHub via Cloudflare Workers Builds — redéploiement automatique à chaque push sur `main`, sans copier-coller manuel de code), secrets configurés, plafond de dépense fixé côté tableau de bord OpenAI, un vrai échange question/réponse observé depuis le chat expert de la PWA.
