...
- **Code réel depuis le 07/09/2026** : `workers/ia-relay/` — fournisseur Claude/Anthropic (conforme au choix "Claude par défaut" ci-dessus), architecture swappable, testé (voir `22-SDS-outil.md` §10quater et `workers/ia-relay/README.md`). Déploiement Cloudflare/Anthropic réel (secrets, plafond de dépense) reste à la charge de l'utilisateur.
