...
### 4.0 Gestion de projets (répond à URS-F-000 à 000undecies, étendu v56 — Phase 37, TD-043 amendée TD-044)
...
- **(étendu v56, Phase 37, TD-043 amendée TD-044)** `Project.owner_id`/`shared_with` (même forme que `Section.owner_id`/`shared_with`, Phase 0 — additif, aucune migration Dexie) — `owner_id` résolu à la création via `identifiantUtilisateurCourant()` (`identiteLocale.ts`) : email du profil local (§4.31/URS-F-310bis) si défini, sinon l'espace réservé Phase 1 inchangé. `partagerProjet`/`retirerPartage` (`useProjectsStore.ts`) ajoutent/retirent une entrée `{user_id, access_level}` — jamais de doublon (mise à jour du niveau si déjà présent) — URS-F-000decies.
- **Garde d'affichage** `peutModifierProjet` (fonction pure, `logique-metier/permissions/permissionsProjet.ts`) : `true` si `owner_id === userId` ou si `shared_with` contient `userId` avec `access_level: 'édition'`, `false` sinon — jamais une vérification de lecture (toujours ouverte). Câblée dans `FicheProjet.vue` : masque archivage, ajout/import de section, import de document, formulaire de partage si l'utilisateur courant n'a pas le droit d'édition — remplace le point ouvert historique de `useProjectsStore.ts`/`FicheProjet.vue` qui attribuait jusqu'ici tout l'audit_log à l'espace réservé Phase 1 quel que soit l'auteur réel — URS-F-000undecies.
...
### 4.1bis Génération de brouillon par adaptation d'un document de référence (répond à URS-F-060 à 064 + 064ter, implémenté v50, Phase 33, TD-031 ; étendu v57, Phase 38 Option 2, TD-045)
...
### 4.12bis Test Design Engine — proposition déterministe depuis l'analyse de risque (répond à URS-F-120octies/nonies, nouveau v55 — Phase 35 de convergence architecturale, TD-036)
...
- **Garde-fou non négociable** : le candidat généré est créé au statut `propose`, identique à un candidat saisi manuellement — mêmes actions Accepter/Rejeter/Besoin d'information, jamais un chemin d'approbation automatique distinct — URS-F-120octies, cohérent avec URS-F-120septies.
- `evaluerCouvertureRisques(requirement, risquesDuClient, candidatsDuClient)` (fonction pure) calcule, pour chaque risque "action requise" du nœud de l'exigence, un statut `couvert`/`non_couvert` — `couvert` uniquement si un `TestCandidate` existe pour ce risque dans un statut actif (`rejete`/`doublon`/`remplace` ne comptent jamais) — URS-F-120nonies.
...
### 4.21 Reasoning Engine — domaine AI (répond à URS-F-210 à sexies + septies, nouveau v32 — Phase 15, TD-007/TD-008 ; étendu v57, Phase 38 Option 1, TD-045)
...
- **(étendu v57, Phase 38 Option 1, TD-045)** Assistant contextuel par section : `EditeurSection.vue` invoque directement `useReasoningEngineStore.executerRaisonnement` (`missionId: null`, `contextSnapshotId: null`) — même fonction que le Mission workspace (§4.23), jamais un second moteur. `construireObjectifAssistantSection` (fonction pure, `logique-metier/raisonnement/assistantSection.ts`) injecte le contenu actuel de la section (valeurs non vides) directement dans le texte de l'objectif — la section elle-même n'étant pas un objet interrogeable par le catalogue d'outils existant (URS-F-210), son contenu est fourni en contexte plutôt que d'étendre ce catalogue pour un seul objet. Historique de conversation local à l'écran (non rechargé depuis le store à la navigation suivante) — `AIRequest`/`AIResponse` restent la trace durable — URS-F-210septies.
...
### 4.32 Parcours unifié « Fiche Client/Site » et navigation regroupée par intention (répond à URS-F-320 à octies, nouveau v58 — Phase 40, 03/09/2026)
...
| URS-F-310 à quinquies | §4.31 |
| URS-F-320 à octies | §4.32 |
