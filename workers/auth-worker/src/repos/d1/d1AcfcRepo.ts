...
import type { ACFCRepo, EvaluationACFCEnregistree, MethodProfileACFCEnregistre } from '../acfcRepo'

function ligneVersProfil(l: Record<string, unknown>): MethodProfileACFCEnregistre {
  return {
    id: l.id as string,
...
function ligneVersEvaluation(l: Record<string, unknown>): EvaluationACFCEnregistree {
  return {
    id: l.id as string,
    clientId: l.client_id as string,
...
export class D1AcfcRepo implements ACFCRepo {
  ...
  async listerEvaluations(clientId: string): Promise<EvaluationACFCEnregistree[]> {
    const resultat = await this.db
      .prepare('SELECT * FROM evaluations_acfc WHERE client_id = ?')
      .bind(clientId)
      .all()
    return resultat.results.map((l) => ligneVersEvaluation(l as Record<string, unknown>))
  }

  async creerEvaluation(e: EvaluationACFCEnregistree): Promise<void> {
    await this.db
      .prepare(
